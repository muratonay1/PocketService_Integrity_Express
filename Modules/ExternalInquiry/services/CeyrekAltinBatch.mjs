...
const CeyrekAltinBatch = executeBatch(async (criteria) => {
     ...
     try {
          ...
          if (satısTL) {
               ...
               const getResponse = await PocketService.executeService(`GetCurrencyWithName`, Modules.EXTERNAL_INQUIRY, searchCurrencyCriteria);
               if (getResponse.data.amount != value) {
                    ...
                    const updateResponse = await PocketService.executeService(`UpdateCurrency`, Modules.EXTERNAL_INQUIRY, updateCurrencyPocket);
                    if (updateResponse) {
